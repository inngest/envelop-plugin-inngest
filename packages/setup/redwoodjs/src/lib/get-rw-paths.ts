 ...
 */
const getConfigPath = (cwd: string = process.env.RWJS_CWD ?? process.cwd()): string => {
 ...
 */
const resolveFile = (filePath: string, extensions: string[] = ['.js', '.tsx', '.ts', '.jsx']): string | null => {
...
export const getRWPaths = (BASE_DIR: string = getBaseDir()): Paths => {
  ...
  const paths = {
    ...
    web: {
      ...
      storybookConfig: path.join(BASE_DIR, PATH_WEB_DIR_CONFIG_STORYBOOK_CONFIG),
      storybookPreviewConfig: path.join(BASE_DIR, PATH_WEB_DIR_CONFIG_STORYBOOK_PREVIEW),
